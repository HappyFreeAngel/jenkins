...
#这是基础镜像,包含了常见的工具集. 2019.03.26 17:32 ok
...
#PATH搜索顺序是从前到后。比较好的习惯是将 导出PATH时将新增的路径放在前面
ENV PATH=/devops/workspace/scripts:$BUILDER_HOME/scripts:$PATH:$SCALA_HOME/bin:$GO_HOME/bin:$GRADLE_HOME/bin:$M2_HOME/bin
...
RUN apk update && apk add --no-cache  openssh  openssh-keygen ansible && sudo pip3 install -r /devops/software/requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple \
    && rm -rf /var/cache/apk/*
